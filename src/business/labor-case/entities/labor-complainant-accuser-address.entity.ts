...
@Entity({ name: 'PC_LABOR_COMPLAINANT_ACCUSER_ADDRESS' })
export class OraclePcLaborComplainantAccuserAddress {
    @PrimaryGeneratedColumn({ name: 'ACCUSER_ADDRESS_ID', comment: 'รหัสระบบบันทึกคดีแรงงาน (รง. 2) ส่วนที่อยู่ผู้ร้อง (AUTO INCREMENT)' }) accuserAddressId: number;
    ...
    @Column({ name: 'COMPLAINANT_ACCUSER_ID', nullable: false, comment: 'รหัสระบบบันทึกคำร้องศาลแรงงาน ส่วนผู้ร้อง เชื่อมโยง PC_LABOR_COMPLAINANT_ACCUSER' }) complainantAccuserId: number;
    ...
    toResponseObject() {
        const { accuserAddressId, createdBy, createdDate, updatedBy, updatedDate, removedBy, removedDate, orderNo, addressNo, addressTypeId, countryId, districtId, complainantAccuserId, moo, phoneNumber, post, provinceId, road, soi, subDistrictId } = this;
        const responseObject = { accuserAddressId, createdBy, createdDate, updatedBy, updatedDate, removedBy, removedDate, orderNo, addressNo, addressTypeId, countryId, districtId, complainantAccuserId, moo, phoneNumber, post, provinceId, road, soi, subDistrictId };
