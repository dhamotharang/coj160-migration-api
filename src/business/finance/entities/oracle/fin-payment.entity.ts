...
export class OracleFinPayments {
  ...
  @Column({ name: "IS_LABOR", default: 0, comment: "" }) isLabor: number;
  ...
  async beforeInsert() {
    try {
      const res = await getManager().query(`SELECT "${process.env.ORA_USERNAME}"."PC_FIN_PAYMENT_SEQ".nextval ID FROM DUAL`);
