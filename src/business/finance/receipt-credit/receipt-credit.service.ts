...
@Injectable()
export class ReceiptCrditService extends HelperService {
  constructor(
    @InjectRepository(OracleFinReceiptCredits)
    private readonly oracleFinReceiptCreditRepositories: Repository<OracleFinReceiptCredits>,
  ) {
    super();
  }
  ...
  async oracleFilter(conditions, filters: any = null, moduleId: number = 0) {
    try {
      if (moduleId !== 0) {
        await conditions.where("A.creaditId = :moduleId", { moduleId });
      } else {
  ...
  async findORACLEData(filters: any = null, pages: any = null, orders: any = null) {
    try {
      const conditions = await this.oracleFinReceiptCreditRepositories.createQueryBuilder("A");
  ...
  // POST Method
  async createData(payloadId: number, data: OracleFinReceiptCreditDTO) {
    try {
      const createdDate = new Date(this.dateFormat("YYYY-MM-DD H:i:s"));
      const created = await this.oracleFinReceiptCreditRepositories.create({
        ...data,
        createdBy: payloadId,
        removedBy: 0,
        createdDate,
      ...
      await this.oracleFinReceiptCreditRepositories.save(created);
